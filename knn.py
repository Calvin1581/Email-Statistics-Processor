...
class KNN:
    ...
    def train(self, data, y):
        ...
        self.exemplars = data
        self.classes = y
        # self.num_classes = np.unique(self.classes)
    ...
    def predict(self, data, k):
        ...
        predictions = np.ndarray(data.shape[0])
        for i in range(data.shape[0]):
            dist = np.ndarray(self.exemplars.shape[0])
            for j in range(self.exemplars.shape[0]):

                n = data[i] - self.exemplars[j]
                n = n**2
                n= n.sum()
                n = math.sqrt(n)
                dist[j] = n
            ...
            dist = dist[0:k]
            for m in range(dist.size):
                dist[m] = self.classes[dist[m]]
            x = stats.mode(dist)
            predictions[i] = x[0]
    ...
    def plot_predictions(self, k, n_sample_pts):
        ...
        flatx = x.flatten()
        flaty = y.flatten()
        stacker = np.column_stack((flatx,flaty))

        pred = self.predict(stacker,k)
        y_pred = np.reshape(pred,(n_sample_pts,n_sample_pts))
        plt.pcolormesh(x, y, y_pred,cmap=colors,shading='auto')
        # plt.scatter(test[:,0],test[:,1],c=test[:,2],cmap = lmcolor1)
